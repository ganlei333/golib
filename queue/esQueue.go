...
func BenchmarkSingleInsertAbsentChan(b *testing.B) {
	var m = make(chan interface{}, b.N)
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		m <- i
	}
...
func BenchmarkSingleInsertAbsentGet(b *testing.B) {
	m := NewQueue(uint32(b.N))
...
func BenchmarkSingleInsertAbsentChanGet(b *testing.B) {
	var m = make(chan interface{}, b.N)
